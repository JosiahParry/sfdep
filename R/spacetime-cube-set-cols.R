...
set_col <- function(x, .from_geo, .to_data = .from_geo) {
  if (!is_spacetime_cube(x)) {
    cli::cli_abort(
      c(
        "`x` must be a valid spacetime cube.",
  ...
  if (is.null(geo_col)) {
    cli::cli_abort("`.from_geo` is missing from `geometry`.")
  }
...
set_wts <- function(x, .wt_col = "wt") {
  ...
  if (is.null(wt)) {
    cli::cli_abort("`.wt_col` is missing from `geometry`.")
  }
...
set_nbs <- function(x, .nb_col = "nb") {
  ...
  if (is.null(nb)) {
    cli::cli_abort("`.nb_col` is missing from `geometry`.")
  }
