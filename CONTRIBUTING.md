...
This outlines how to propose a change to sfdep. For more detailed info
...
- Implementation of new functionality should first be developed to work
  with `spdep`. If a function relies on neighbors and weights, the
  function should utilize a `listw` object.
- If accepting multiple columns of input, accept lists
- If providing multiple columns of output, utilize `data.frames` (not
  tibbles)
...
- Fork the package and clone onto your computer. If you haven’t done
  ...
  `usethis::create_from_github("JosiahParry/sfdep", fork = TRUE)`.
...
Please note that the sfdep project is released with a [Contributor Code
of Conduct](https://josiahparry.github.io/sfdep/CODE_OF_CONDUCT.md). By
